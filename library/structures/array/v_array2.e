...
feature {NONE} -- Initialization
	...
	make (n, m: INTEGER)
		...
		do
			...
			matrix_rows_effect: matrix.domain |=| (create {MML_INTERVAL}.from_tuple ([1, n]))
			matrix_columns_effect: matrix.range.for_all (agent (r: MML_SEQUENCE [G]; m_: INTEGER): BOOLEAN
				do
					Result := (r.domain |=| (create {MML_INTERVAL}.from_tuple ([1, m_]))) and r.is_constant (({G}).default)
				end (?, m))
		end
	...
	make_filled (n, m: INTEGER; v: G)
		...
		do
			...
			matrix_columns_effect: matrix.range.for_all (agent (r: MML_SEQUENCE [G]; m_: INTEGER; v_: G): BOOLEAN
				do
					Result := (r.domain |=| (create {MML_INTERVAL}.from_tuple ([1, m_]))) and r.is_constant (v_)
				end (?, m, v))
		end
...
invariant
	...
	map_domain_definition: map.domain |=| ( create {MML_INTERVAL}.from_tuple ([1, row_count * column_count]))
